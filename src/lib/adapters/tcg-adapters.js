// @ts-nocheck
/* eslint-disable */
// Portiert aus public/adapters-browser.js — TCG-Such-Engine (externe APIs).
// Bewusst 1:1 uebernommen; nur Modul-Wrapper angepasst.
...
// TCGdex, Scryfall, YGOPRODeck und optcgapi.com sind dafür ausgelegt (getestet).
  ...
  function readPromoLS() {
    try {
      const raw = globalThis.localStorage && globalThis.localStorage.getItem(OP_PROMO_LS);
      if (!raw) return null;
  ...
  function writePromoLS(cards) {
    try { if (globalThis.localStorage) globalThis.localStorage.setItem(OP_PROMO_LS, JSON.stringify({ at: Date.now(), cards })); } catch {}
  }
