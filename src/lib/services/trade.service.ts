...
// Kaufanfrage: ich (proposer) moechte die Karte von responder (Verkaeufer).
export async function createTrade(t: {
	...
	const row = {
		proposer: me, responder: t.responder, card_id: t.cardId, card_name: t.cardName,
		card_game: t.cardGame ?? null, price: t.price ?? null, currency: t.currency || 'EUR', message: t.message ?? null
	};
...
export async function updateTrade(
	id: number,
	fields: { status?: TradeStatus; proposer_done?: boolean; responder_done?: boolean; price?: number }
): Promise<void> {
	const patch: Record<string, unknown> = {};
	for (const k of ['status', 'proposer_done', 'responder_done', 'price'] as const) {
...
// Anzahl offener eingehender Kaufanfragen (fuer Badge).
