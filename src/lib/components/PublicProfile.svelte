<script lang="ts">
  ...
  import { getPublicProfile, type PublicProfile } from '$lib/services/profile.service';
  ...
  $effect(() => {
    ...
    if (!id) { prof = null; offers = []; err = ''; return; }
    loading = true; err = ''; prof = null; offers = [];
    (async () => {
      try {
...
{#if profileView.userId}
  <div class="ov" onclick={close} role="presentation">
    <div class="dlg" role="dialog" aria-modal="true" onclick={(e) => e.stopPropagation()}>
      ...
      {#if loading}
        <div class="pad">Lädt…</div>
      {:else if err}
        <div class="pad err">{err}</div>
      {:else if prof}
        <div class="phead">
          <h2>{prof.name ?? 'Sammler'}</h2>
          <div class="sub">
            {#if prof.country}{prof.country} · {/if}Mitglied seit {since(prof.member_since)}
          </div>
          <div class="stats">
            {#if prof.rank != null}<span class="stat">Rang #{prof.rank}</span>{/if}
            {#if prof.points != null}<span class="stat">{prof.points} Punkte</span>{/if}
            {#if prof.rating_avg != null}<span class="stat">★ {prof.rating_avg} ({prof.rating_count ?? 0})</span>{/if}
          ...
          {#if prof.fav_games || prof.collector_type}
            <div class="meta2">{[prof.collector_type, prof.fav_games].filter(Boolean).join(' · ')}</div>
          {/if}
        ...
        {#if offers.length === 0}
          <div class="pad muted">Keine aktiven Angebote.</div>
        {:else}
          <div class="ogrid">
...
<style>
  ...
  .sub { color: var(--muted); display: flex; align-items: center; gap: 2px; font-size: 0.9rem; }
  ...
  .bio { margin: 10px 0; }
  .contact { color: var(--accent, #6366f1); font-size: 0.85rem; }
