<script lang="ts">
  ...
  let importing = $state(false); let importMsg = $state('');
  ...
  async function onImport(e: Event) {
    ...
    try {
      ...
      await load();
    } catch (err) { importMsg = (err as Error).message; }
...
<div class="exports">
  ...
  <button onclick={exportXlsx}>Excel (.xlsx) exportieren</button>
  <button onclick={printList}>Bestandsliste drucken</button>
</div>
